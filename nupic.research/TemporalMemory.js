...
TemporalMemory.prototype = {
    ...
    init: function(c) { // void(Connections)
        var matrix = util.isNullOrUndefined(c.getMemory()) ?
        ...
        for (var i = 0; i < numColumns; i++) {
            ...
            if (util.isNullOrUndefined(colZero)) {
                matrix.set(i, column);
            }
        }
...
//console.log(connections.getActiveSynapsesForSegment());
        var result = this.computeFn(connections, connections.getColumnSet(activeColumns), new Set(connections.getPredictiveCells()),
            new Set(connections.getActiveSegments()), util.copyOf(connections.getActiveSynapsesForSegment(), "map"),
...
//console.log(prevActiveSegments);
    ...
    burstColumns: function(cycle, c, activeColumns, predictedColumns,
        ...
        for (var column of activeColumns) {
            ...
            if (!util.isNullOrUndefined(bestCell)) {
                cycle.winnerCells.add(bestCell);
            }

            var segmentCounter = c.getSegmentCount();
            if (util.isNullOrUndefined(bestSegment)) {
...
//console.log(prevActiveSynapseSegments);
