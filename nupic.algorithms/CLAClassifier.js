...
CLAClassifier.prototype = {
    ...
    compute: function(recordNum, classification, patternNZ, learn, infer) { // <T> ClassifierResult<T>(int, Map<String, Object>, int[], boolean, boolean)
        ...
        if (infer) {
            ...
            var actValues = util.newArray([this.actualValues.length], 0);
            for (var i = 0; i < this.actualValues.length; i++) {
                actValues[i] = util.isNullOrUndefined(this.actualValues[i]) ? defaultValue : this.actualValues[i];
            }
            ...
            for (var i = 0; i < this.steps.length; i++) {
                ...
                var sumVotes = util.newArray([this.maxBucketIdx + 1], 0);
                var bitVotes = util.newArray([this.maxBucketIdx + 1], 0);
                ...
                for (var j = 0; j < patternNZ.length; j++) {
                    ...
                    if (util.isNullOrUndefined(history)) {
                        continue;
                    }
        ...
        if (learn && !util.isNullOrUndefined(classification.get("bucketIdx"))) {
            ...
            if (util.isNullOrUndefined(this.actualValues[bucketIdx])) {
        ...
        if (infer && this.verbosity >= 1) {
            ...
            for (var i = 0; i < retVal.stepSet().length; i++) {
                ...
                if (util.isNullOrUndefined(retVal.getActualValue(key))) {
                    continue;
                }
