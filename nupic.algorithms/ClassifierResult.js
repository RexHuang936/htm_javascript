...
ClassifierResult.prototype = {
    ...
    copy: function() { // ClassifierResult<T>(void)
        ...
        retVal.actualValues = util.copyOf(this.actualValues, "array");
        retVal.probabilities = util.copyOf(this.probabilities, "map"); // new TIntObjectHashMap<double[]>(probabilities);
    ...
    getActualValue: function(bucketIndex) { // T(int)
        if (util.isNullOrUndefined(this.actualValues) || this.actualValues.length < bucketIndex + 1) {
            return null;
        }
    ...
    getMostProbableValue: function(step) { // T(int)
        var idx = -1;
        if (util.isNullOrUndefined(this.probabilities.get(step)) || (idx = this.getMostProbableBucketIndex(step)) === -1) {
            return null;
        }
    ...
    getMostProbableBucketIndex: function(step) { // int(int)
        if (util.isNullOrUndefined(this.probabilities.get(step))) {
            return -1;
        }
    ...
    hashCode: function() { // int(void)
        ...
        result = prime * result + (util.isNullOrUndefined(this.probabilities) ? 0 : HashCode.value(this.probabilities));
        return result;
    },
    ...
    equals: function(obj) { // boolean(Object)
        ...
        if (util.isNullOrUndefined(this.probabilities)) {
            if (!util.isNullOrUndefined(other.probabilities)) {
                return false;
            }
