...
QuickTest.prototype = {
    ...
    main: function(args) { // void(String[])
        ...
        var classifier = new CLAClassifier(util.newArray([1], 1), 0.1, 0.3, 0);
...
console.log("encoding="+value+":"+encoding);
        ...
        if (!util.isNullOrUndefined(this.predictedColumns)) {
            this.lastPredicted = util.copyOf(this.predictedColumns, "array");
        }
    ...
    inflateSDR: function(SDR, len) { // int[](int[], int)
        var retVal = util.newArray([len], 0);
    ...
    getSDR: function(cells) { // int[](Set<Cell>)
        var retVal = util.newArray([cells.size], 0);
        var i = 0;
    ...
    switch (event.data.action) {
		case "start":
			...
			importScripts(
				...
				url + "cipun/util.js",
				url + "cipun/LinkedBlockingDeque.js",
				url + "cipun/LoggerFactory.js",
